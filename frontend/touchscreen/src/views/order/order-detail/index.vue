<template>
  <div class="order_detail">
    <div class="order-goods">
      ...
      <van-cell-group>
        <van-cell title="商品金额">
          <span class="red">{{orderInfo.goodsPrice * 100 | euro}}</span>
        </van-cell>
        <van-cell title="快递费用">
          <span class="red">{{orderInfo.freightPrice * 100 | euro}}</span>
        </van-cell>
      </van-cell-group>
    </div>
    ...
    <van-cell-group style="margin-top: 20px;">
      ...
      <van-cell title="实付款：">
        <span class="red">{{orderInfo.actualPrice * 100 | euro}}</span>
      </van-cell>
      <!-- 订单动作 -->
      <van-cell>
        <van-button
...
import { orderDetail, orderDelete, orderConfirm, orderRefund } from '@/api/api';
